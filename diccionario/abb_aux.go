...
func (a *abb[K, V]) abbBuscar(clave K, anterior, actual **nodoABB[K, V]) (encontrado bool, padre, nodo **nodoABB[K, V]) {
	if *actual == nil {
		return false, anterior, actual
	}

	comparacion := a.cmp(clave, (*actual).clave)
	if comparacion == 0 {
		return true, anterior, actual
	}
	if comparacion < 0 {
...
func (a *abb[K, V]) encontrarSucesor(anterior, actual *nodoABB[K, V]) (padreSucesor, sucesor *nodoABB[K, V]) {
	if anterior.izq == nil {
